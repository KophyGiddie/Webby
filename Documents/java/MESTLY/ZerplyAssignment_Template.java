...
public class ZerplyAssignment_Template 
{
	...
	static void userMenu () throws Exception {
		...
		else if (answer.equals("n")) 
		{
			...
			checkNoInfo(email, password, splittedInput,PROFILE_FILE);
		}
	}

	static void checkNoInfo (String email, String password, String [] userInfo, String filename) throws Exception
	{
		Scanner input = new Scanner(System.in);
		if (userLogIn(email, password, userInfo)) 
			{
				printUserProfile(filename, email);
			}
		else 
		{
			while(!userLogIn(email, password, userInfo))
			{
				System.out.println("Your email and password do not match please enter again");
				...
				{
					while((!tryAgain.equals("n")) && (tryAgain.equals("y")))
					{
						System.out.println("Wrong entry. You must enter (y/n)");
						System.out.println("Do you want to try again (y/n)");
...
{ 
		if (input.matches("www\\.[a-z0-9]+\\.[a-z0-9]+\\.[a-z]{2,3}")) {
			return true;
		} else {
			return false;
		}
